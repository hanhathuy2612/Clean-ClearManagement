...
EXEC USP_timKiemHoaDonThuTienTheoTen N'Trương' 
...
EXEC USP_timKiemHoaDonThuTienTheoID 2
...
EXEC USP_timKiemHoaDonThuTienTheoIDvaTEN 2, N'T'
...
END
-- TESST
EXEC USP_Login N'xuong', N'4'

EXEC dbo.USP_GetTotal_NhanVien

EXEC dbo.USP_GetListHoaDonThuTien

exec USP_GetHangTheoDanhMuc 2
...
SELECT * FROM dbo.CHITIETHD WHERE ID_HD = 1




SELECT ID_HD,NgayNhap,NgayKetThuc,Express,TrangThaiGiac,TrangThaiTT FROM dbo.HOADON -- hd, dbo.KHACHHANG kh WHERE hd.ID_KH = kh.ID_KH

UPDATE dbo.HOADON SET TrangThaiTT = N'Xong', NgayKetThuc = GETDATE() WHERE ID_HD = 2

	SELECT DISTINCT * FROM dbo.HOADON hd,dbo.KHACHHANG kh WHERE hd.ID_KH = kh.ID_KH AND  kh.TenKH LIKE N'%Trương%'

	SELECT hd.ID_HD,hd.NgayNhap,hd.NgayKetThuc,kh.TenKH, nv.TenNV, hd.TrangThaiGiac, hd.TongTien , hd.TrangThaiTT FROM dbo.HOADON hd, dbo.KHACHHANG kh, dbo.NHANVIEN nv
	WHERE hd.ID_KH = kh.ID_KH AND hd.ID_NV = nv.ID_NV AND hd.TrangThaiGiac = N'Xong' AND kh.TenKH LIKE N'%'+ N'Trương' +'%'
